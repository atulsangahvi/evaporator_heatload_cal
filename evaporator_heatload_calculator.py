...
P_evap_bar = st.sidebar.number_input("evaporating Pressure (bar abs)", value=2.00, min_value=1.0, max_value=35.0, step=0.1)
...
try:
    ...
    if T3 < T_sat:
        h1 = PropsSI("H", "P", P_evap, "T", T3, fluid)
    else:
    ...
    if T1 > T_sat:
        h4 = PropsSI("H", "P", P_evap, "T", T1, fluid)
    else:
        h4 = h3
